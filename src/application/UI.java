...
public class UI {
	...
	private static void printCapturedPieces(List <ChessPiece> captured) { //uso stream pra filtrar a lista apenas com a cor q quero
		List <ChessPiece> white = captured.stream().filter(x -> x.getColor() == Color.WHITE).collect(Collectors.toList()); //expressão lambda
